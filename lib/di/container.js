...
module.exports = class Container {
  ...
  createFactory(svc, opts) {
    ...
    switch (type) {
      case 'class': return this.createClassFactory(svc, metadata.paramNames)
      case 'factory': return this.createFunctionFactory(svc, metadata.paramNames)
      default: return () => svc
    }
  }

  createClassFactory(klass, paramNames) {
    return () => createInstance(klass, true, paramNames, this)
  }

  createFunctionFactory(svc, paramNames) {
    return () => createInstance(svc, false, paramNames, this)
  }
}

function createInstance(Func, isConstructor, paramNames, resolver) {
  let paramValues = paramNames.map(_ => resolver.get(_, { allowAsync: true }))
