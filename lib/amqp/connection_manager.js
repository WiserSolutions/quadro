...
module.exports = class AmqpConnectionManager extends EventEmitter {
  ...
  async close() {
    ...
    this._channels = []
    this._closeConnection().catch((err) => {})
    this.emit('close')
  ...
  async _connect(wait = false) {
    ...
    this._connectPromise = new Promise(r => setTimeout(r, wait ? this.reconnectTimeout : 0))
  ...
  async _connectInner() {
    ...
    const url = (await urlsByDistance(this.urls))[0]
    let conn = null
    ...
    conn.on('error', () => {
      // Need this here to "catch" the error; close emitted after error, so we can ignore it.
    })
