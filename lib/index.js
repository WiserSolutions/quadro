...
function aggregatedMetricsServerHandler(req, res) {
  if (req.method === 'GET' && req.url === '/metrics') {
    this.aggr.clusterMetrics((err, metrics) => {
      if (err) {
        Q.log.warn(err)
        res.writeHead(500, 'Content-Type', 'text/plain')
        res.end(err.message)
      } else {
        res.writeHead(200, 'Content-Type', this.aggr.contentType)
        res.end(metrics)
      }
    })
    res.writeHead(200, { 'Content-Type': 'text/plain' })
...
async function boot(opts) {
  ...
  if (cluster.isMaster && clustering) {
    for (let i = 0; i < os.cpus(); ++i) cluster.fork()
  ...
  if (clustering && cluster.isMaster) {
    const aggr = new promClient.AggregatorRegistry()
    http.createServer(aggregatedMetricsServerHandler.bind({aggr}))
