...
class Logger {
  ...
  async initialize() {
    ...
    this._metricLogger = this.createAuxiliaryStream('metrics')
    this._eventLogger = this.createAuxiliaryStream('events')

    await this.reload()
  }

  createAuxiliaryStream(filename) {
    return bunyan.createLogger({
      name: this.app.name,
      level: 'info',
      streams: [{ path: path.resolve(this.logsDir, `${filename}.log`) }]
    })
  }
  ...
  */
  metric(name, dimensions, values) {
    this._metricLogger.info({ d: { metric: name, ...dimensions }, ...values })
  }
